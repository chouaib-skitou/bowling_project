...
#TEST
    ...
    def check_first_combo_value(*args):
        if combo_Score_value.get() == str(nb_skittles): #strike donc pas de second lancé
            combo_Score_second.config(state="disabled")
        else:
    ...
    # Validation
    btnValider = Button(frame, text="Valider", command=lambda: [majScore((combo_Score_first.get(),combo_Score_second.get()),current_player_indice),change_player(entries, int(nb_tours), nb_skittles, window,int(current_player_indice))]) #verif_Skittles(combo_Score.get())
